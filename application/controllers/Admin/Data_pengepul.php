...
class Data_pengepul extends CI_Controller {
	...
	public function index(){
		$data = array('role' => $this->session->userdata('role'), 'title' => "Dashboard Admin", 'nama' => $this->session->userdata('nama'), 'state'=> 'peng');
		$data["data"] = $this->User_model->getAccountPengepul();
		$this->load->view('layout/header_after_login', $data);
		$this->load->view('layout/navbar_after_login', $data);
		$this->load->view('admin/layout/sidebar', $data);
		$this->load->view('admin/content_pengepul', $data);
	...
	public function proses_addUser(){
		...
		if($hasil){
			echo "<script type='text/javascript'>alert('Berhasil menambahkan pengguna'); window.location.href='../data_pengepul'</script>";
		}
		else
	...
	public function updateDataUser($id){
		...
		if($hasil){
			echo "<script type='text/javascript'>alert('Berhasil updatedata pengguna'); window.location.href='..'</script>";
		}
		else
			echo "<script type='text/javascript'>history.go(-1);</script>";
	}
	...
	public function deleteUser($id){
		...
		if($hasil){
			echo "<script type='text/javascript'>alert('Berhasil menghapus pengguna'); window.location.href='..'</script>";
